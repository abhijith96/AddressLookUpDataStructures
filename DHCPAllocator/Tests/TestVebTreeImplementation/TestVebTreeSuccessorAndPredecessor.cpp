...
protected:
    ...
    bool CheckIfSuccessorsMatch(const std::vector<veb_key_t>& keys, const std::vector<veb_key_t>& sucKeys, const std::vector<veb_value_t>& sucValues){
        for(size_t i = 0; i < keys.size(); ++i){
            veb_key_t  key = keys[i];
    ...
    bool CheckIfPredecessorsMatch(const std::vector<veb_key_t>& keys, const std::vector<veb_key_t>& predKeys, const std::vector<veb_value_t>& predValues){
        for(size_t i = 0; i < keys.size(); ++i){
            veb_key_t  key = keys[i];
            VEBTreeNodeKeyType nodeKeyType;
            veb_key_t predKey;
...
TEST_F(VEBTest, SuccesorOfTreeWithFourElements) {
    ...
    std::vector<veb_key_t> sucKeysOutput = {1,1,5,5,5,5,9,9,9,9,13,13,13,13};
...
TEST_F(VEBTest, SuccesorOfTreeWithUniverse256) {
    ...
    for(int i = 1; i <= 10; ++i) {
        ...
        std::vector<veb_key_t> sucKeysOutput = GenerateSeries(10, 24, 10);
